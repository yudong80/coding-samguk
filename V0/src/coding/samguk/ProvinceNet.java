    ...
    private void init() { 
        ...
        Province[] arr = {
            ...
            new Province(11, "낙양", 조조, 조조, 순욱, 782_000, 3_500, 65_000, 40, 57, 0, 60, 56, 730, 40)
                .addGeneral(gMap.find("조조").setArmy(6500, 65, 60))
                .addGeneral(gMap.find("곽가"))
                .addGeneral(gMap.find("하우연").setArmy(2300, 65, 60))
                .addGeneral(gMap.find("하우은"))
                .addGeneral(gMap.find("하우모").setArmy(1500, 65, 60))
                .addGeneral(gMap.find("순욱").setArmy(1600, 65, 60))
                .addGeneral(gMap.find("순상"))
                .addGeneral(gMap.find("순유"))
                .addGeneral(gMap.find("조앙").setArmy(1000, 65, 60))
                .addGeneral(gMap.find("조홍").setArmy(2000, 65, 60))
                .addGeneral(gMap.find("조인").setArmy(2100, 65, 60))
                .addGeneral(gMap.find("조순").setArmy(1800, 65, 60))
                .addGeneral(gMap.find("진교"))
                .addGeneral(gMap.find("정욱"))
                .addGeneral(gMap.find("전위").setArmy(2300, 65, 60))
                .addGeneral(gMap.find("이전").setArmy(1800, 65, 60))
                .addGeneral(gMap.find("유연")),
            new Province(12, "홍농", 조조, 하우돈, 만총, 141_000, 2_700, 55_000, 55, 55, 0, 55, 42, 580, 40)
                .addGeneral(gMap.find("하우돈").setArmy(6400, 55, 59))
                .addGeneral(gMap.find("우금").setArmy(1500, 55, 59))
                .addGeneral(gMap.find("윤봉").setArmy(1700, 55, 59))
                .addGeneral(gMap.find("악진"))
                .addGeneral(gMap.find("강서").setArmy(1900, 55, 59))
                .addGeneral(gMap.find("차주").setArmy(1000, 55, 59))
                .addGeneral(gMap.find("장연").setArmy(1800, 55, 59))
                .addGeneral(gMap.find("조앙").setArmy(1800, 55, 59))
                .addGeneral(gMap.find("만총"))
                .addGeneral(gMap.find("모개").setArmy(1500, 55, 59))
                .addGeneral(gMap.find("양부").setArmy(1700, 55, 59))
                .addGeneral(gMap.find("양관").setArmy(1600, 55, 59)), 
